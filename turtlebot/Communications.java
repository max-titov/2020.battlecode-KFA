...
public class Communications {
	...
	//messages
	final int M_ATTACK_READY = 234;
	final int M_DEFENSE_READY = 233;
	...
	final int M_UBER = 839;
	final int M_UBER_COMPLETED = 139;
    ...
    public MapLocation getEnemyHQLocFromBlockchain() throws GameActionException {
    	int[] enemyMessage = checkForMessage(M_ENEMY_HQ_LOC);
		if(enemyMessage != null) {
			return new MapLocation(enemyMessage[1], enemyMessage[2]);
		}
		return null;
    }
    ...
    public void requestUber(MapLocation loc, MapLocation desiredLoc) throws GameActionException {
    	int[] m = {M_UBER,loc.x,loc.y,desiredLoc.x,desiredLoc.y,rc.getID()};
    	sendMessage(m,1);
    }
    
    public void uberCompleted(MapLocation loc, MapLocation desiredLoc, int id) throws GameActionException {
    	int[] m = {M_UBER_COMPLETED,loc.x,loc.y,desiredLoc.x,desiredLoc.y,id};
    	sendMessage(m,1);
    }
}
